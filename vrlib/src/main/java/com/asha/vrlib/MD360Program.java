...
public class MD360Program {
    ...
    public void build(Context context){
        final String vertexShader = getVertexShader(context,  mContentType);
    ...
    protected String getVertexShader(Context context, int type){
        int resId;
        switch (type){
            case MDVRLibrary.ContentType.CUBE_BITMAP:
            case MDVRLibrary.ContentType.CUBE_VIDEO:
                resId = R.raw.cube_per_pixel_vertex_shader;
                break;
            case MDVRLibrary.ContentType.BITMAP:
            case MDVRLibrary.ContentType.VIDEO:
            default:
                resId = R.raw.per_pixel_vertex_shader;
    ...
    private static class FragmentShaderFactory{
        ...
        static String fs(Context context, int type){
            ...
            switch (type){
                case MDVRLibrary.ContentType.CUBE_BITMAP:
                    resId = R.raw.cube_per_pixel_fragment_shader_bitmap;
                    resId = R.raw.per_pixel_fragment_shader_bitmap;
                     break;
                case MDVRLibrary.ContentType.CUBE_VIDEO:
                    resId = R.raw.cube_per_pixel_fragment_shader;
                    break;
                case MDVRLibrary.ContentType.BITMAP:
                    resId = R.raw.per_pixel_fragment_shader_bitmap;
                    break;
